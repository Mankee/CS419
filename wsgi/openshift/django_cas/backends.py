        ...
        if tree[0].tag.endswith('authenticationSuccess'):
            for element in tree[0]:
                if element.tag.endswith('user'):
                    ...
                    logging.error('attempting to login ' + username)
        elif tree[0].tag.endswith('authenticationFailure'):
            logging.error("error validating ticket" + ticket + ' ' + response)
        else:
            logging.error('error parsing XML from from CAS')
        return username
    except urllib2.URLError as e:
        logging.error('error opening URL for scraping' + e)
    except urllib2.HTTPError as e:
        logging.error('server could not fulfill request' + e)
    finally:
        page.close()
